...
document.getElementById("venueTypeFilter").addEventListener("change", () => {
  sortResults();
  displayPage(1);
});
...
window.addEventListener("load", () => {
  ...
  if (saved) {
    const { location, radius, day, evening } = JSON.parse(saved);
    document.getElementById("venueLocation").value = location || "";
    document.getElementById("searchRadius").value = radius || "5";
    document.getElementById("venueDayGuests").value = day || "";
    document.getElementById("venueEveningGuests").value = evening || "";
  }
});
...
document.getElementById("venueSearchForm").addEventListener("submit", async function (e) {
  ...
  filteredVenues = venues.map(venue => {
    ...
    if (typeFilter && venue.type !== typeFilter) return false;
...
<p><strong>Venue Hire:</strong> ${venue.venue_price}</p>
    ...
    `;
    if (
      dayGuests >= venue.min_day_guests &&
      dayGuests <= venue.max_day_guests &&
      dayGuests + eveningGuests <= venue.max_evening_guests
    ) {
