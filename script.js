...
function initAutocomplete() {
  ...
  if (stored) {
    const { location, radius } = JSON.parse(stored);
...
function displayPage(page) {
  ...
  document.getElementById("resultsSummary").innerHTML = `Showing ${venuesToShow.length} of ${filteredVenues.length} venues:`;
  ...
  (typeof venue.venue_hire === 'number' ? venue.venue_hire : parseFloat(venue.venue_hire) || 0)
    ...
    div.innerHTML = `
      <h2>${venue.name}</h2>
      <p><strong>Location:</strong> ${venue.location} (approx. ${venue.distance.toFixed(1)} miles)</p>
