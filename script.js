...
function displayPage(page) {
  ...
  venuesToShow.forEach(venue => {
    ...
    const totalCost = useEstimate ? (
      parseInt(venue.price_per_day_guest.replace('£', '').replace('pp', '').trim()) * parseInt(document.getElementById("venueDayGuests").value) +
      parseInt(venue.price_per_evening_guest.replace('£', '').replace('pp', '').trim()) * parseInt(document.getElementById("venueEveningGuests").value) +
      (parseInt(venue.venue_hire) || 0)
    ) : 0;
